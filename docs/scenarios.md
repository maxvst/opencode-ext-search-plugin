...
Если параметр `directories` отсутствует или пуст — плагин возвращает пустой объект и завершает работу. Хуки и tools не регистрируются, поведение OpenCode не изменяется.
...
После разрешения каждая директория проверяется на существование. Несуществующие или неявляющиеся-директориями — пропускаются с предупреждением в лог.

Если после фильтрации resolvedDirs пуст — плагин завершает работу без регистрации хуков.

### 1.4. Обнаружение ripgrep

На этом же этапе плагин ищет бинарник `rg` в системе. Результат сохраняется и используется при обработке `grep`.
...
Если библиотека `zod` не обнаружена — tool `deps_read` не регистрируется, плагин логирует предупреждение.
