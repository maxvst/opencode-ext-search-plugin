...
Авто-permit, автоматическое разрешение (auto-permit) — механизм автоматического одобрения запросов OpenCode на доступ к внешним директориям (`external_directory`), пути которых находятся внутри настроенных внешних директорий (resolvedDirs). Поскольку пользователь явно указал внешние директории в конфигурации плагина, это подразумевает согласие на доступ к файлам в этих директориях. Подробнее см. [Авто-permit](scenarios/auto-permit.md).

Обработчик авто-permit (auto-permit handler) — функция, создаваемая `createAutoPermitHandler`, регистрируется как хук `event` плагина. Перехватывает событие `permission.asked` и автоматически одобряет доступ, если запрошенный путь находится внутри resolvedDirs. Подробнее см. [Авто-permit](scenarios/auto-permit.md).

`createAutoPermitHandler` — фабричная функция (`auto-permit.ts`), создающая обработчик событий для авто-permit. Принимает resolvedDirs и client, возвращает async-функцию для хука `event`. Подробнее см. [Авто-permit](scenarios/auto-permit.md#создание-обработчика).

`shouldAutoApprove` — функция (`auto-permit.ts`), определяющая, следует ли автоматически одобрить запрос разрешения. Проверяет тип разрешения (`external_directory`) и наличие запрошенных путей внутри resolvedDirs. Подробнее см. [Авто-permit](scenarios/auto-permit.md#логика-принятия-решения).
