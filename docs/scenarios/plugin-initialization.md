...
Если параметр `directories` отсутствует или пуст, параметр `compile_commands_dir` не указан **и** файл `.ext-search.json` не найден в configDir — плагин показывает toast-уведомление (`warning`) с описанием проблемы, возвращает пустой объект и завершает работу. Хуки и tools не регистрируются, поведение OpenCode не изменяется.

Таким образом, плагин может быть активирован исключительно через `.ext-search.json` без указания `directories` или `compile_commands_dir` в `opencode.json`.
...
## Чтение пользовательских директорий из .ext-search.json

Если найден configDir и в нём присутствует файл `.ext-search.json`, плагин вызывает `parseUserDirs` для извлечения пользовательских директорий.

> **Примечание:** если `configDir` равен `null` (т.е. `opencode.json` не найден при обходе от openDir), парсинг `.ext-search.json` молча пропускается — `parseUserDirs` не вызывается. Плагин продолжает работу без user-директорий.

Подробное описание алгоритма см. в [Пользовательские директории из .ext-search.json](user-dirs.md).

Результатом является массив `ExternalDir[]` с `source: "user"`. При ошибках парсинга или невалидных директориях показываются toast-уведомления (`error`), но плагин продолжает работу с уже разрешёнными config- и cc-директориями.

## Объединение и фильтрация директорий

Config-директории, cc-директории и user-директории объединяются в единый массив `allDirs: ExternalDir[]` в порядке: config, cc, user. Затем выполняется:

1. **Пометка disabled (config/cc)** — config-директория помечается `disabled: true`, если она является поддиректорией одной из cc-директорий (функция `markDisabledConfigDirs`). Это исключает дублирование результатов: cc-директория покрывает более широкую область.
2. **Пометка disabled (user)** — любая config- или cc-директория помечается `disabled: true`, если она является поддиректорией одной из user-директорий (функция `markDisabledByUserDirs`). User-директория покрывает более широкую область.
3. **Формирование activeDirPaths** — массив путей всех не-disabled директорий: `allDirs.filter(d => !d.disabled).map(d => d.path)`.
4. **Проверка активности** — если `activeDirPaths` пуст, показывается toast (`warning`) и плагин завершает работу.
