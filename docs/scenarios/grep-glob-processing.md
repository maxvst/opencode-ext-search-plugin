...
flowchart TD
    ...
    B -->|Да| C["collectGlobBun: Bun.Glob.scan по каждой директории (до maxPerDir в каждой)"]
    B -->|Нет| D["collectGlobFsWalk: рекурсивный обход walkDir по каждой директории"]
    C --> E["Фильтрация через shouldExclude"]
    D --> E
    E --> F["Сбор файлов со всех директорий"]
    F --> G["Ограничение вывода до effectiveMax"]
    G --> H["Вычисление hintDirs: директории с усечёнными результатами"]
```

- **Bun.Glob** (`collectGlobBun`) — используется, если runtime предоставляет `Bun.Glob`. Паттерн передаётся нативно. Обходит **все** директории, ограничивая до `maxPerDir` файлов в каждой.
- **walkDir** (`collectGlobFsWalk`) — fallback при отсутствии Bun. Выполняет рекурсивный обход каждой директории через `fs.readdirSync`, фильтруя через `shouldExclude`. Паттерн при этом **не применяется** — возвращаются все файлы, не попавшие под исключения. Обходит **все** директории с отдельным ограничением на каждую.

**Примечание:** при использовании walkDir-fallback glob-паттерн не фильтрует результаты, поэтому количество файлов может быть больше, чем при использовании Bun.Glob. Ограничение `maxResults` применяется к общему выводу после обхода всех директорий.
