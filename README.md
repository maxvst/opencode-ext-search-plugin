...
**Если бюджет > 0, но внешних результатов больше бюджета:** выводятся сколько влезло + та же подсказка, чтобы ИИ знал, что во внешних директориях есть дополнительные совпадения, и мог использовать `deps_read` или `grep`/`glob` с явным указанием пути.
